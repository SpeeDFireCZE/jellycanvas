...
- Cards: the play button on posters has its own settings (Cards section, "Play button on posters"): style (Jellyfin's own, accent, dark, white, glass), a color of its own, corner radius, position (Jellyfin's own, middle or a corner - a bottom corner starts above an overlaid title, and on the web above the small mark / favourite / menu group), size, and "no play button on phones". Applies to the web's hover button and the fixed one touch clients draw; the TV has none, so the rows stay out of the TV view.
