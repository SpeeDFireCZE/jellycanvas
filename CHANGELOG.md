...
- Card badges on small cards (a phone, a dense grid): the two top corners shrink a little to sit side by side before one is moved past the other, the badges shrink as a whole (their padding follows the font), the inset is smaller, and the shrinking is decisive (two rounds) so a hair of overlap no longer sends a corner under the other; a long sound name (TrueHD Atmos 7.1) goes short (Atmos 7.1) before a corner has to move. Test media: Oppenheimer carries TrueHD 5.1 next to four flags and four codes. A column at the top - under a played tick, or moved past the other corner - now ends above the title strip and above the play button touch clients keep on every poster (a column a little too long first shrinks to fit, down to about three quarters; one still too long lies down into rows; only then does it drop extra flags, then badges); a play button placed in a top corner pushes that corner's badges under it, and bottom badges stay above a bottom one.
