...
(function () {
    ...
    var strings = {
        ...
        cs: {
            ...
            bannerHeading: 'Banner', bannerHint: 'Vlastní obrázek položky za její stránkou – tak, jak to dělá „banner na detailu“ v Jellyfinu, ale z tématu, takže ho vidí všichni bez ohledu na nastavené pozadí.', bannerOn: 'Obrázek položky za její stránkou', bannerImage: 'Obrázek', bannerTop: 'Držet horní část obrázku',
        ...
        page.querySelectorAll('[data-slider]').forEach(function (row) {
            ...
            function commit(v, fromNumber) {
                ...
                if (path.indexOf('Scripts.') === 0) {
                    ...
                    schedulePreview();
                    if (path.indexOf('InfoBar.') === 0 || path.indexOf('Backdrop.') === 0 || path.indexOf('Detail.Banner') === 0 || path === 'Tv.StaticBackdrop') {
    ...
    function refreshConditions() {
        ...
        // "Still background on TV" leaves the rotation and the panning with
        // nothing to do there, so the TV view does not offer them - it says
        // where they went instead.
        var tvStill = editDevice === 'Tv' && !!getPath(view(), 'Tv.StaticBackdrop');
        page.querySelectorAll('[data-not-still]').forEach(function (el) {
            el.hidden = el.hidden || tvStill;
        });
        page.querySelectorAll('[data-still-hint]').forEach(function (el) {
            el.hidden = !tvStill;
        });
    }
