...
(function () {
    ...
    function renderSeerrRows() {
        ...
        list.forEach(function (r, i) {
            ...
            // "Movie / Series" on the posters says nothing when the row is
            // limited to one of them - the tick goes away with the choice.
            var mediaSel = node.querySelector('[data-field="Media"]');
            var typeTick = node.querySelector('[data-field="ShowType"]').closest('label');
            var syncType = function () { typeTick.hidden = mediaSel.value !== 'Both'; };
            mediaSel.addEventListener('change', syncType);
            syncType();
            node.querySelector('.jc-btn-remove').addEventListener('click', function () {
